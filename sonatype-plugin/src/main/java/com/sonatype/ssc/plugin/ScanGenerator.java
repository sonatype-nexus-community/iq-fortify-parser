...
public class ScanGenerator {
  ...
  private ScanGenerator(final Random random, final File outputFile, final int issueCount, final int categoryCount,
      final int longTextSize, final Instant now) {
    this.random = random;
  ...
  public static void main(String[] args) throws NoSuchAlgorithmException, IOException, InterruptedException {
    ...
    if (!argsOk) {
      LOG.error(String.format(
          "Usage:\n" + "\tjava -cp <class_path> %s " + SCAN_TYPE_FIXED + " <OUTPUT_SCAN_ZIP_NAME>\n"
              ...
              + " <OUTPUT_SCAN_ZIP_NAME> <ISSUE_COUNT> <CATEGORY_COUNT> <LONG_TEXT_SIZE>\n",
    ...
    if (isScanFixed()) {
      ...
      scanGenerator = new ScanGenerator(SecureRandom.getInstanceStrong(), new File(args[1]), Integer.valueOf(args[2]),
          Integer.valueOf(args[3]), Integer.valueOf(args[4]), Instant.now());
    }
    scanGenerator.write();
  }
...
//        jsonGenerator.writeStringField(WEBSITE.attrName(),fn.getWebsite());
  ...
  private static byte[] loremIpsum = ("Lorem ipsum dolor sit amet, eam ridens cetero iuvaret id. Ius eros fabulas ei. Te vis unum intellegam, cu sed ullum eruditi, et est lorem volumus. Te altera malorum quaestio mei, sea ea veniam disputando.\n"
      + "\n"
